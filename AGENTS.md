...
Every file-changing turn must make one conventional microcommit, while frontend changes must also rebuild with `desktop:dist`, reinstall with `scripts/install-desktop-app.sh` (never hand-roll a `mv` backup — the script keeps one rollback copy at a fixed path so /Applications does not fill with stale bundles), relaunch it, and confirm `GET /api/desktop-health` returns 200.
