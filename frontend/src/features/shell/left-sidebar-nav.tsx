...
import { Activity, Clock, FlaskConical, ServerCog, TrendingUp } from "@/ui/icon-registry";
...
export function mobilePageTitle(pathname: string): string {
  ...
  if (pathname.startsWith("/science")) return "Scientific workbench";
  if (pathname.startsWith("/logs")) return "Logs";
  const tab = tabs.find((entry) => isRouteActive(pathname, entry.href));
  return tab?.label ?? BRAND_PROFILE.appName;
}
