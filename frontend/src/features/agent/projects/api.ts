...
import { ProjectResponseSchema, ProjectsResponseSchema } from "@shared/agent/projects";
...
export type OpenProjectDirectoryResult =
  | { source: "desktop"; path: string | null }
  | { source: "fallback" };
...
export async function openProjectDirectory(): Promise<OpenProjectDirectoryResult> {
  ...
  if (typeof picker !== "function") return { source: "fallback" };
  return { source: "desktop", path: await picker() };
}
