...
export function AgentChatPaneHeader({
  ...
  // Every menu action closes the menu; the item just says what it does.
        ...
        <div ref={ref} className="relative flex min-w-0 items-center gap-1.5">
          ...
          {open ? (
            <div className={CHAT_HEADER_MENU_CLASS} role="menu">
              ...
              <MenuItem onClick={runAndClose(onTogglePinned)}>{pinned ? "Unpin" : "Pin"}</MenuItem>
              <MenuItem disabled={!canFork} onClick={runAndClose(onFork)}>
                Fork
              </MenuItem>
              <MenuItem disabled={!canExport || !onExport} onClick={runAndClose(onExport)}>
                Export as Markdown
              </MenuItem>
              <MenuItem onClick={runAndClose(() => setReasoningVisible(!reasoningVisible))}>
