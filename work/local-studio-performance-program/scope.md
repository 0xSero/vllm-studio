...
| Cluster topology | `/gpus` runs coordinator-local `nvidia-smi` only, and the GB10 zero-memory fallback substitutes host `totalmem`/`freemem` in `controller/src/modules/system/platform/gpu.ts`, while `controller/src/modules/compute/devices/host.ts` reports the host-memory side of the possible double count. | 2x/4x Sparks collapse into one coordinator GB10 and unified memory risks double counting. [Task 13](tasks/task-13.md) owns truthful networked 2x/4x topology: per-node/per-device VRAM, utilization, temperature, and power with provenance, named unified pools counted once, and Local Studio UI acceptance. |
| Vision capability | `resolveModelVision` detection exists in `controller/contracts/model-capabilities.ts`; pairing, routing, persistence, failure semantics, and attribution do not. | [Task 14](tasks/task-14.md) owns the end-user Local Studio pairing flow: same-controller/same-node persistence, proxy routing, exact-instance attribution, and the mandatory composer flow. |
...
4. Every expected node and device reports VRAM used and total, utilization, temperature, and power. Every telemetry sample carries node provenance and sampled-at staleness; aggregates over missing or stale nodes are explicitly partial, and an unavailable metric renders unavailable, never zero. Log streams are scoped `controller|node|instance` with the origin on every line, and the Local Studio status/log surfaces expose every available origin.
...
7. Vision pairing is a persisted same-controller primary/vision reference pair, routed and attributed server-side in the controller OpenAI proxy with typed fail-closed errors and no implicit fallback. Pairing requires the same `ControllerRef` and the same `NodeId` at the serving-endpoint level — resolved primary and sidecar instances have equal `InstanceRecord.nodeId`, a mismatch is rejected at write time with a typed error naming both node IDs, and cross-node pairing inside a multi-node controller is a named continuation behind an explicit configuration flag, never a silent default. The pairing is configured by the end user in Local Studio against a primary that may lack vision; the UI shows both exact model identities and the pairing state; attribution records the exact selected instance without re-resolving from model name; missing, stopped, capability-incompatible, cross-controller, and cross-node pairings fail closed with typed visible errors; and the composer flow (14b) is mandatory for [Task 14](tasks/task-14.md) acceptance.
...
| 04:30–06:30 | Wave 3 — Tasks 03, 07, 14a: Electron timeline/stores; Usage UI; vision-sidecar pairing/routing/attribution (the mandatory 14b follows 03r — in Wave 4 if Task 03 slips — and Task 14 passes only when 14b passes). Tasks 13 and 14 run in parallel only after their respective gates — 13 after 12c, 14a after the strict 12r -> 06f -> 14a sequence — and with disjoint ownership. | Cross-repo contract versions and immutable pins agree; assistant and serving telemetry stay separate. | Contract goldens, routing/attribution fixtures, desktop source proof. |
...
| Topology and vision | Every expected node/device shows VRAM used and total, utilization, temperature, and power with provenance and staleness; unavailable metrics show unavailable, never zero; aggregates label missing/stale nodes as partial; each GB10 unified pool is counted once; the Local Studio UI matches per-device and aggregate fixture values; image parts route to the paired sidecar and text to the primary with exact-instance attribution; the composer keeps image bodies under a healthy pairing. |
