...
- Topology fixtures pass for 1-node, 2-node, and 4-node Sparks and the Pop 1-node/4-GPU shape with correct controller/node/device counts; every expected node/device shows VRAM used and total, utilization, temperature, and power with provenance and staleness, and unavailable metrics show unavailable rather than zero; each GB10 unified pool is counted once; the Local Studio UI matches the per-device and aggregate values; live Spark read-only surfaces end `PASS` or truthfully `BLOCKED` with exact per-node reachability and missing fields (R18, [Task 13](task-13.md)).
- Vision pairing passes only as the complete end-user flow: the pairing is configured and shown with both exact model identities in Local Studio, image parts route to the sidecar and text to the primary with exact-instance attribution never re-resolved from model name, and the mandatory 14b composer flow passes; anything less leaves R20 unaccepted with the exact gap recorded (R20, [Task 14](task-14.md)).
