...
const expectForbidden = (response: Response): Effect.Effect<void, Error, never> =>
  Effect.tryPromise({
    try: () => response.json(),
    catch: (error) => Error(String(error)),
  }).pipe(
    Effect.map((body) => {
      ...
      expect(body).toEqual({ detail: "Forbidden request origin" });
    }),
    Effect.asVoid,
  );
...
describe("keyless controller boundary", () => {
  test("rejects hostile authority and origin before logging or route execution", () =>
    runControllerEffect(
      runtime,
      Effect.gen(function* () {
        const evictionsBefore = evictCalls;
        const logsBefore = requestLogCalls;
        yield* requestEviction({
          host: "localhost:18080",
          origin: "https://attacker.example",
        }).pipe(Effect.flatMap(expectForbidden));
        yield* requestEviction({
  ...
  test("allows configured browser and native loopback authorities", () =>
    runControllerEffect(
      runtime,
      Effect.gen(function* () {
        const evictionsBefore = evictCalls;
        ...
        for (const entry of cases) {
          const response = yield* requestEviction(entry);
  ...
  test("fails closed for malformed request values", () =>
    runControllerEffect(
      runtime,
      Effect.gen(function* () {
        const evictionsBefore = evictCalls;
        ...
        for (const entry of cases) {
          yield* requestEviction(entry).pipe(Effect.flatMap(expectForbidden));
        }
        expect(evictCalls).toBe(evictionsBefore);
      }),
    ));
  ...
  test("enforces concrete and wildcard LAN authority lists", () =>
    runControllerEffect(
      runtime,
      Effect.gen(function* () {
        const evictionsBefore = evictCalls;
        ...
        const allowedResponse = yield* requestEviction(
          { host: "192.168.1.10:18080", origin: "https://studio.lan" },
          lanApp,
        );
        ...
        const wildcardResponse = yield* requestEviction(
