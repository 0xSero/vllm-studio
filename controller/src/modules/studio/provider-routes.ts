...
export const registerStudioProviderRoutes = defineRoutes((app, context) => {
  return mergeRoutes(
    effectRoute.get(app, "/studio/providers", (ctx) =>
      Effect.sync(() => ctx.json({ providers: context.config.providers.map(serializeProvider) })),
    ),
    effectRoute.post(app, "/studio/providers", (ctx) =>
      Effect.gen(function* () {
    ...
    effectRoute.put(app, "/studio/providers/:id", (ctx) =>
      Effect.gen(function* () {
    ...
    effectRoute.delete(app, "/studio/providers/:id", (ctx) =>
      Effect.gen(function* () {
        const providerId = ctx.req.param("id") ?? "";
    ...
    effectRoute.get(app, "/studio/provider-models", (ctx) =>
      Effect.forEach(
