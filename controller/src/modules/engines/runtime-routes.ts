...
export const registerRuntimeRoutes = defineRoutes((app, context) => {
  ...
  return mergeRoutes(
    effectRoute.get(app, "/runtime/targets", (ctx) =>
      Effect.gen(function* () {
    ...
    effectRoute.post(app, "/runtime/targets/:targetId/select", (ctx) =>
      Effect.gen(function* () {
    ...
    effectRoute.post(app, "/runtime/jobs", (ctx) =>
      Effect.gen(function* () {
    ...
    effectRoute.get(app, "/runtime/vllm", (ctx) =>
      getVllmRuntimeInfo().pipe(Effect.map((info) => ctx.json(info))),
    ),
    effectRoute.get(app, "/runtime/vllm/config", (ctx) =>
      getVllmConfigHelp().pipe(Effect.map((config) => ctx.json(config))),
    ),
    effectRoute.get(app, "/runtime/llamacpp/config", (ctx) => {
    ...
    effectRoute.get(app, "/runtime/sglang", (ctx) =>
      Effect.gen(function* () {
    ...
    effectRoute.get(app, "/runtime/llamacpp", (ctx) =>
      Effect.gen(function* () {
    ...
    effectRoute.get(app, "/runtime/rocm", (ctx) =>
      getRocmInfo(resolveRocmSmiTool()).pipe(Effect.map((info) => ctx.json(info))),
    ),
    effectRoute.post(app, "/runtime/:backend/upgrade", (ctx) =>
      Effect.gen(function* () {
