...
const LogLimitQuerySchema = Schema.Struct({
  limit: Schema.optionalKey(
    Schema.FiniteFromString.pipe(
      Schema.check(Schema.isInt(), Schema.isBetween({ minimum: 1, maximum: 20000 })),
    ),
  ),
});
const LogTailQuerySchema = Schema.Struct({
  tail: Schema.optionalKey(
    Schema.FiniteFromString.pipe(
      Schema.check(Schema.isInt(), Schema.isBetween({ minimum: 0, maximum: 20000 })),
    ),
  ),
});
...
const terminateChild = (child: ReturnType<typeof spawn>): Effect.Effect<void> =>
  Effect.gen(function* () {
    ...
    const exited = yield* Effect.raceFirst(
      ...
      Effect.sleep(1000).pipe(Effect.as(false)),
    );
    ...
    yield* Effect.raceFirst(waitForChildExit(child), Effect.sleep(1000));
  });
export const registerLogsRoutes = defineRoutes((app, context) => {
  ...
  return mergeRoutes(
    ...
    effectRoute.get(app, "/logs/:sessionId", (ctx) =>
      Effect.gen(function* () {
    ...
    effectRoute.delete(app, "/logs/:sessionId", (ctx) =>
      Effect.gen(function* () {
        const sessionId = yield* decodeSessionId(ctx.req.param("sessionId") ?? "");
        if (sessionId === "controller") {
    ...
    effectRoute.get(app, "/events", (ctx) =>
      Effect.sync(() => {
        ...
        return new Response(toReadableByteStream(withSseHeartbeat(frames, 15000, signal)), {
          headers: buildSseHeaders(),
        });
      }),
    ),
    effectRoute.get(app, "/logs/:sessionId/stream", (ctx) =>
      Effect.gen(function* () {
        const sessionId = yield* decodeSessionId(ctx.req.param("sessionId") ?? "");
