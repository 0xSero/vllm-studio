...
/** No runtime in the payload means "the engine's pinned image": docker with a
 *  bare engine ref, which the launch path treats as image-from-spec. llama.cpp
 *  defaults to the native `llama-server` on PATH instead. */
const defaultRuntime = (backend: unknown): Record<string, unknown> => {
  ...
  return runtimeReference === "llamacpp"
    ...
    : { kind: "docker", ref: runtimeReference };
};
...
export const recipeSchema = Schema.Struct({
  ...
  backend: Schema.Literals(["vllm", "sglang", "exllamav3", "llamacpp"]),
