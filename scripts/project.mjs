...
var init_assert_standalone_build = __esm(() => {
  projectRoot = resolve(import.meta.dirname, "../frontend"), standaloneBase = resolve(projectRoot, ".next", "standalone"), candidates = [
...
var init_perf_routes = __esm(() => {
  scriptsDir = dirname(fileURLToPath(import.meta.url)), appDir = join(scriptsDir, "..", "frontend", "src", "app"), preferredOrder = [
    "/",
    "/agent",
    "/agent/sessions",
    "/settings",
...
var init_complete_standalone_build = __esm(() => {
  projectRoot2 = resolve2(import.meta.dirname, "../frontend"), repoRoot = resolve2(projectRoot2, ".."), standaloneBase2 = resolve2(projectRoot2, ".next", "standalone"), standaloneRoots = [resolve2(standaloneBase2, "frontend"), standaloneBase2], standaloneRoot = standaloneRoots.find((root) => existsSync5(resolve2(root, "server.js")));
  if (!standaloneRoot)
...
var init_desktop_package_smoke = __esm(async () => {
  require3 = createRequire3(path4.resolve(path4.dirname(fileURLToPath3(import.meta.url)), "../frontend/package.json")), { chromium } = require3("playwright-core");
...
var init_link_services_node_modules = __esm(() => {
  frontendDir = path5.resolve(path5.dirname(fileURLToPath4(import.meta.url)), "../frontend"), servicesDir = path5.join(path5.dirname(frontendDir), "services"), linkPath = path5.join(servicesDir, "node_modules");
...
var init_patch_pi_ai_openai_text_boundaries = __esm(() => {
  frontendRoot = path6.resolve(path6.dirname(fileURLToPath5(import.meta.url)), "../frontend"), targetFiles = [
...
var init_prepare_next_build = __esm(() => {
  rmSync6(resolve3(import.meta.dirname, "../frontend/.next"), { recursive: !0, force: !0 });
});
...
var init_start_standalone = __esm(async () => {
  projectRoot3 = resolve4(dirname3(fileURLToPath10(import.meta.url)), "../frontend"), standaloneRoot2 = resolve4(projectRoot3, ".next", "standalone"), nestedRoot = resolve4(standaloneRoot2, "frontend"), serverRoot = existsSync12(nestedRoot) ? nestedRoot : standaloneRoot2, rawPort = process.env.PORT || "4783", port = Number(rawPort);
...
var init_validate_package_json = __esm(() => {
  pkgPath = resolve6(import.meta.dirname, "../frontend/package.json"), pkg = JSON.parse(readFileSync15(pkgPath, "utf8")), required = ["scripts", "devDependencies"], requiredScripts = ["dev", "build", "desktop:dist"], missing = [];
...
var init_validate_ui_structure = __esm(() => {
  projectRoot4 = resolve8(import.meta.dirname, "../frontend"), srcRoot = join6(projectRoot4, "src"), legacyPrimitivePurityFiles = new Set([]), sharedLayerAllowlist = new Set([]), retiredUiFeatureDirs = new Set([
    "recipes",
    "discover",
    "configs",
    "usage",
...
function auditLayout() {
  let expected = ["scripts/install-controller.sh", "scripts/install-desktop-app.sh", "scripts/project.mjs"], actual = readdirSync10(path11.join(root5, "scripts"), { withFileTypes: !0 }).filter((entry) => entry.isFile()).map((entry) => `scripts/${entry.name}`).sort(), executable = git(["ls-files", "-s"]).split("\n").filter((line) => line.startsWith("100755 ")).map((line) => line.split("\t")[1]).sort(), stale = ["frontend/scripts", "controller/scripts", "services/agent-runtime/scripts"].filter((directory) => existsSync(path11.join(root5, directory)));
  if (JSON.stringify(actual) !== JSON.stringify(expected) || JSON.stringify(executable) !== JSON.stringify(expected) || stale.length > 0)
...
`).reduce((total, row) => {
    ...
    if (!/^\d+$/.test(added ?? "") || !/^\d+$/.test(removed ?? ""))
      return total;
    if (file2 === "scripts/project.mjs" || !existsSync(path11.join(root5, file2 ?? "")) || /(^|\/)(package-lock\.json|bun\.lockb?|.*\.snap)$/.test(file2 ?? ""))
      return total;
...
if (invoked === "commit-msg")
  ...
  prePush();
else if (path11.resolve(process.argv[1] ?? "") === fileURLToPath11(import.meta.url)) {
