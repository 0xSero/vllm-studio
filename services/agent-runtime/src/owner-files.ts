...
function openOwnerFile(filePath: string): number {
  if (!safeExistingDirectory(path.dirname(filePath))) {
...
export function ownedSourceFileExists(filePath: string): boolean {
  try {
    if (!ownerFileExists(filePath)) return false;
    return (lstatSync(filePath).mode & 0o022) === 0;
  } catch {
    return false;
  }
}
