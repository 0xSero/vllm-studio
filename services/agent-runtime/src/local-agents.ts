...
import { isRecord } from "./local-agent-value";
import {
  backupExistingFile,
  existingFileMode,
  pathExists,
...
export async function revokeAgentAttachments(home: string, results: AttachResult[]): Promise<void> {
  const root = `${path.resolve(home)}${path.sep}`;
  const restore = async (configPath: string, backupPath?: string) => {
    const resolved = path.resolve(configPath);
    if (!resolved.startsWith(root)) throw new Error("Agent configuration path escapes home");
    if (backupPath) {
      const resolvedBackup = path.resolve(backupPath);
      if (!resolvedBackup.startsWith(root)) throw new Error("Agent backup path escapes home");
      if ((await lstat(resolvedBackup)).isSymbolicLink()) {
        throw new Error("Agent backup must not be a symbolic link");
      }
      await rename(resolvedBackup, resolved);
      return;
    }
  ...
  for (const result of [...results].reverse()) {
    ...
    for (const extra of [...(result.extraUpdates ?? [])].reverse()) {
      await restore(extra.configPath, extra.backupPath);
    }
    await restore(result.configPath, result.backupPath);
  }
}
