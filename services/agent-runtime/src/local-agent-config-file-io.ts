...
export async function backupExistingFile(file: string): Promise<string> {
  if ((await lstat(file)).isSymbolicLink()) {
    throw new Error(`Refusing to back up symbolic link: ${file}`);
  }
  const base = `${file}.bak-local-studio-${backupTimestamp(new Date())}`;
  for (let suffix = 1; suffix < 10_000; suffix += 1) {
    const backupPath = suffix === 1 ? base : `${base}-${suffix}`;
    try {
  ...
  throw new Error(`Unable to allocate backup path for ${file}`);
}
