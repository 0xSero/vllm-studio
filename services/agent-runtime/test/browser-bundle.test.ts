...
test("packages external runtimes without build-machine paths", () => {
  ...
  expect(runtime.grpc).toStartWith(path.join(packageDir, "dist", "node_modules"));
  expect(runtime.protobuf).toStartWith(path.join(packageDir, "dist", "node_modules"));
}, 30_000);
